...
const props = defineProps({ //変数の受け取り
  user: String,
})
let form = useForm({
                    ...
                    <div class="w-full px-3 sm:w-1/2">
                      <div class="mb-5">
                        ...
                        <input v-model="form.name" type="text" name="name" id="name" placeholder="名前"
                          class="w-full border border-[#e0e0e0] bg-white py-3 px-6 outline-none focus:border-[#6A64F1] focus:shadow-md" 
                          :class="{'border-red-300': form.errors.name}"/>
                        <div v-show="form.errors.name" class="text-red-400">{{ form.errors.name }}</div>
                      </div>
                    </div>
                    <div class="w-full px-3 sm:w-1/2">
                      <div class="mb-5">
                        ...
                        <input v-model="form.email" type="text" name="email" id="email" placeholder="アドレス"
                          class="w-full border border-[#e0e0e0] bg-white py-3 px-6 outline-none focus:border-[#6A64F1] focus:shadow-md" 
                          v-bind:class="{'border-red-300': form.errors.email}"/>
                        <div v-show="form.errors.email" class="text-red-400">{{ form.errors.email }}</div>
                      </div>
                    </div>
                  </div>
