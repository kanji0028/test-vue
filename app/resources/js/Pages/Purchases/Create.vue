...
//ページ読み込み後、即座に実行
onMounted(() => {
  form.date = getToday(); //関数getTodayの実行結果をform.dateに入れる
  props.items.forEach(item => { //配列を1つずつ処理 itemはdefinePropsないのitemsの1件ずつの情報
...
const itemList = ref([]) //リアクティブのカラム配列

//リアクティブでフォームの情報をオブジェクト形式で登録
const form = reactive({
  date: null,
  customer_id: null,
  status: true,
                    ...
                    <div class="flex flex-wrap -m-2">
                      ...
                      <div class="p-2 w-full">
                        <div class="relative">
                          ...
                          <MicroModal @update:customerId="setCustomId"/>
                          <!-- <select type="text" id="customer" name="customer" v-model="form.customer_id"
                            class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
